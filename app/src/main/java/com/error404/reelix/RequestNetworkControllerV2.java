...
public class RequestNetworkControllerV2 {
	...
	private static RequestNetworkControllerV2 mInstance;
	
	public static synchronized RequestNetworkControllerV2 getInstance() {
		if (mInstance == null) {
			mInstance = new RequestNetworkControllerV2();
		}
		return mInstance;
	}
	...
	public void execute(final RequestNetworkV2 requestNetwork, String method, String url, final String tag, final RequestNetworkV2.RequestListener requestListener) {
		...
		try {
			if (requestNetwork.getRequestType() == REQUEST_PARAM) {
				...
				// REQUEST_BODY: prefer a raw pre-built JSON string if one was supplied,
				// since Gson can't correctly serialize org.json.JSONArray/JSONObject values
				// nested inside a HashMap<String, Object>.
				String jsonPayload;
				if (requestNetwork.hasRawJsonBody()) {
					jsonPayload = requestNetwork.getRawJsonBody();
				} else {
					jsonPayload = new Gson().toJson(requestNetwork.getParams());
				}
				
				RequestBody reqBody = RequestBody.create(MediaType.parse("application/json"), jsonPayload);
